...
- **Master trading log chart refresh (local branch):** `tools\finalize-master-workbook.ps1` now trims chart source ranges to rows whose X value and at least one series value have real, non-error data. The first two trade charts, `Cumulative P&L (SOL)` and `P&L % per Trade`, also set category-axis tick label spacing from the usable data-point count so small logs show every label and larger logs thin labels automatically. Validation used a temp copy of `E:\Apps\WilyTrader Captures\master trading log.xlsx`; finalization completed with no warnings, chart 1 and chart 2 saved explicit tick spacing, and `npm test` passed with 24 tests.
...
- **Screenshot cancel + launcher hotkey fallback (local branch):**
 ...
 - Screenshot region selection now hides the launcher immediately, not just fullscreen screenshot capture, so Snipalot does not obscure the target while the user draws the selection. Cancel via `Escape`/hotkey or a completed capture restores the launcher through the existing idle visibility path.
 - `v1.1.17` is the release vehicle for the screenshot region-selection launcher hide fix; installed builds before that still show `1.1.16` and will not include the behavior.
