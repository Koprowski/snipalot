...
- **NICS scoring standard update (2026-05-19):**
 - `src/main/trade-pipeline.ts` now prompts Gemini/API extraction to score N/I/C/S separately. v1.0.48 corrected the counted-trade rule to the user's stated 4-point standard: `NICS_score = N_score + I_score + C_score + S_score`, and Core NICS++ requires a score of 4 before separate size/zone/cooldown checks.
