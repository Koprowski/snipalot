...
- **NICS scoring standard update (2026-05-19):**
 - `src/main/trade-pipeline.ts` now prompts Gemini/API extraction to score N/I/C/S separately but compute `NICS_score` as the 3-part unlock score: `N_score + I_score + max(C_score, S_score)`. A counted trade requires N + I + either C or S; C is the actual cut/close reason, and S is the sell/stay plan for working trades.
